...
void HallReverb::ensureBufferCapacity(int numSamples)
{
    if (numSamples <= 0)
    {
        return;
    ...
    leftBufferIn.resize(requiredSize);
    rightBufferIn.resize(requiredSize);
    leftEarlyOut.resize(requiredSize);
    rightEarlyOut.resize(requiredSize);
    leftLateIn.resize(requiredSize);
    rightLateIn.resize(requiredSize);
    leftLateOut.resize(requiredSize);
    rightLateOut.resize(requiredSize);
}
                         ...
                         float* rightChannelOut, int numSamples)
{
    ...
    {
        leftBufferIn[static_cast<size_t>(i)] = leftChannelIn[i];
        rightBufferIn[static_cast<size_t>(i)] = rightChannelIn[i];
    }

    early.processreplace(leftBufferIn.data(),
                         rightBufferIn.data(),
                         leftEarlyOut.data(),
                         rightEarlyOut.data(),
                         numSamples);
    ...
    for (int i = 0; i < numSamples; ++i)
    {
        leftLateIn[static_cast<size_t>(i)] =
            earlySendLevel * leftEarlyOut[static_cast<size_t>(i)] +
            leftBufferIn[static_cast<size_t>(i)];
        rightLateIn[static_cast<size_t>(i)] =
            earlySendLevel * rightEarlyOut[static_cast<size_t>(i)] +
            rightBufferIn[static_cast<size_t>(i)];
    }

    late.processreplace(leftLateIn.data(),
                        rightLateIn.data(),
                        leftLateOut.data(),
                        rightLateOut.data(),
                        numSamples);
    ...
    for (int i = 0; i < numSamples; ++i)
    {
        leftChannelOut[i] = dryLevel * leftBufferIn[static_cast<size_t>(i)] +
                            earlyLevel * leftEarlyOut[static_cast<size_t>(i)] +
                            lateLevel * leftLateOut[static_cast<size_t>(i)];
        rightChannelOut[i] = dryLevel * rightBufferIn[static_cast<size_t>(i)] +
                             earlyLevel * rightEarlyOut[static_cast<size_t>(i)] +
                             lateLevel * rightLateOut[static_cast<size_t>(i)];
    }
}
